...
const system_prompts = (personality = 'friendly') => {
    ...
    return {
        journal_entry:
            prefix + `summarize what you are told like a journal. Put all of your points in bullet points`,
        action_items:
...
export async function get_summary(text, personality) {
    const summary = await query_llm(system_prompts(personality).journal_entry, text);
    return summary;
}

export async function get_actions(text, personality) {
    const actions = await query_llm(system_prompts(personality).action_items, text);
    return actions;
}

export async function get_response(text, personality) {
    const response = await query_llm(system_prompts(personality).dino_response, text);
    return response;
}

export async function get_help(text, personality) {
    const help = await query_llm(system_prompts(personality).stress_help, text);
    return help;
}
